...
(def-struct/cond-contract c ([S Type?] [T Type?]) #:transparent)
...
(def-struct/cond-contract dcon ([fixed (listof c?)] [rest (or/c c? #f)]) #:transparent)

;; fixed : Listof[c]
;; rest : c
(def-struct/cond-contract dcon-exact ([fixed (listof c?)] [rest c?]) #:transparent)

;; fixed : Listof[c]
;; type : c
;; bound : var
(def-struct/cond-contract dcon-dotted ([fixed (listof c?)] [type c?] [bound symbol?]) #:transparent)

(define-for-cond-contract dcon/c (or/c dcon? dcon-exact? dcon-dotted?))

;; map : hash mapping index variables to dcons
(def-struct/cond-contract dmap ([map (hash/c symbol? dcon/c #:flat? #t)]) #:transparent)
...
(def-struct/cond-contract cset ([maps (listof (cons/c (hash/c symbol? c? #:immutable #t #:flat? #t) dmap?))]) #:transparent)
