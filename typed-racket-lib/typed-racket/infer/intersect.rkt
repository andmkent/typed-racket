  ...
  (let intersect ([t1 t1] [t2 t2] [seen seen] [obj obj])
    ...
    (match* (t1 t2)
      ...
      [((Prefab: key1 flds1) (Prefab: key2 flds2))
       #:when (and (not (prefab-key/mutable-fields? key1))
       ...
       (define flds* (for/list ([fty1 (in-list flds1)]
                                [fty2 (in-list flds2)]
                                [n (in-naturals)])
                       (rec fty1 fty2 (-prefab-idx-of key1 n obj))))
       (cond
         [(ormap Bottom? flds*) -Bottom]
         [else (make-Prefab key1 (append flds* (list-tail flds1 (length flds*))))])]
      [((Prefab: key1 flds1) (Prefab: key2 flds2))
       #:when (and (not (prefab-key/mutable-fields? key1))
                   (not (prefab-key/mutable-fields? key2))
       ...
       (define flds* (for/list ([fty1 (in-list flds1)]
                                [fty2 (in-list flds2)]
                                [n (in-naturals)])
                       (rec fty1 fty2 (-prefab-idx-of key2 n obj))))
       (cond
         [(ormap Bottom? flds*) -Bottom]
         [else (make-Prefab key2 (append flds* (list-tail flds2 (length flds*))))])]
      [((Syntax: t1*) (Syntax: t2*))
